...
const {
  ...
  isSettingAsWallaper,
  isAskingExplaination,
  getExplanationCachedFile,
  getFilePath,
} = require("./utils");
const {
  downloadWallpaper,
  cacheExplanationFile,
  displayExplanation,
} = require("./logic");

const apiUrl =
  "https://api.nasa.gov/planetary/apod?api_key=xu6OTf9PpXlJW0LrFpCxXys55XtQKcHK80sQuVuW&thumbs=true";
...
const getApodExplanation = () => {
  ...
  getExplanationCachedFile().then((response) => {
    if (response) {
      ...
      request(apiUrl, async (err, res, body) => {
        ...
        if (res.statusCode === 200) {
          ...
          handleError(
            `Error, Response send with status code ${res.statusCode}`
          );
        }
      });
    }
...
switch (true) {
  ...
  default:
    ...
    console.log(
      "You need an env variable named APOD_WALLPAPER which is an absolute filepath to a directory which store the wallpaper"
    );
    console.log(
      'e.g APOD_WALLPAPER="/absolute/file/path" apod-wallpaper set | explain'
    );
    break;
}
